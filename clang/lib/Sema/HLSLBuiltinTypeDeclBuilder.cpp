...
public:
  ...
  template <typename T> BuiltinTypeMethodBuilder &callHandleCtor(T Handle);
...
Expr *BuiltinTypeMethodBuilder::convertPlaceholder(PlaceHolder PH) {
  ...
  if (PH >= PlaceHolder::LocalVar_0) {
    unsigned LocalVarIndex = static_cast<unsigned>(PH) -
                             static_cast<unsigned>(PlaceHolder::LocalVar_0);
    assert(LocalVarIndex < LocalVars.size() && "local var index out of range");
    VarDecl *VD = LocalVars[LocalVarIndex];
...
BuiltinTypeDeclBuilder &BuiltinTypeDeclBuilder::addCreateFromImplicitBinding() {
  ...
  return BuiltinTypeMethodBuilder(*this, "__createFromImplicitBinding",
                                  AST.VoidTy, false, false, AS_public,
                                  SC_Static)
      .addParam("orderId", AST.UnsignedIntTy)
      .addParam("spaceNo", AST.UnsignedIntTy)
      .addParam("range", AST.IntTy)
