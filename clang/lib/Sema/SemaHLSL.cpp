...
static bool DiagnoseLocalRegisterBinding(Sema &S, SourceLocation &ArgLoc,
  ...
  if (Ty->isRecordType())
    // RecordTypes will be diagnosed at the end of the translation unit;
    return true;
...
ProcessResourceBindingOnUserRecordDecl(const RecordType *RT,
  ...
  while (!TypesToScan.empty()) {
    ...
    if (T->isIntegralOrEnumerationType() || T->isFloatingType()) {
      if (RegType == RegisterType::C)
        // CBuffer should not be handled here?
        return true;
    }
...
// Walks though existing expliting bindings, finds the actual resource class
// decl the binding applies to and updates RegisterBinding::ResourceClass
// and RegisterBinding::Size accordingly.
